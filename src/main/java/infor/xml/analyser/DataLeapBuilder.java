...
public class DataLeapBuilder {
    ...
    public JsonObject build(){
        ...
        try {
            ...
            setReaderWriterColumns(conf);
            root.getAsJsonArray("tasks").get(0).getAsJsonObject().addProperty("conf",conf.toString());
            logbook.success("构建JSON");
        } catch (Exception e) {
            logger.error("出现异常",e);
            logbook.seriousError("构建JSON",e.getMessage());
        }
        return root;
    }
     ...
     * @return
     */
    private void setReaderWriterColumns(JsonObject conf){
        JsonArray sourceColumns=new JsonArray();
        JsonArray targetColumns=new JsonArray();
        //获取转换字段
        Map<String,String> transformFields=getTransformFields();
        Map<String,String> sourceFields=getSourceFields();
        Map<String,String> targetFields=getTargetFields();
        //获取链接
        List<Connector> connectors=this.folder.getMapping().getConnectors();
        //获取实例
        List<Instance> instances=this.folder.getMapping().getInstances();
        ...
        Instance transInstance=instances.stream().filter(item->"TRANSFORMATION".equals(item.getTYPE())).findFirst().orElse(null);
    ...
    private Map<String,String> getTransformFields(){
        return this.folder.getMapping().getTransformations().get(0).getTransformfields()
                .stream().collect(Collectors.toMap(Transformation.Transformfield::getNAME, Transformation.Transformfield::getDATATYPE));
    }

    private Map<String,String> getSourceFields(){
        return this.folder.getSource().getSourcefields()
                .stream().collect(Collectors.toMap(Source.Sourcefield::getNAME, Source.Sourcefield::getDATATYPE));
    }

    private Map<String,String> getTargetFields(){
        return this.folder.getTarget().getTargetfields()
                .stream().collect(Collectors.toMap(Target.Targetfield::getNAME, Target.Targetfield::getDATATYPE));
    }
